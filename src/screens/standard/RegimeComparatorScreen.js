...
export default function RegimeComparatorScreen() {
  const [form, setForm] = useState({
    grossSalary: '', hra: '', hraExempt: '',
    deduction80C: '', deduction80D: '', nps80CCD: '', homeLoanInterest: '',
  });
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.intro}>
          FY 2025-26. New-regime standard deduction is 75,000. Old regime allows 80C, 80D, HRA, and home-loan deductions.
        </Text>

        <Accordion
          title="Your income"
          subtitle={form.grossSalary ? `Gross INR ${parseFloat(form.grossSalary).toLocaleString()}` : 'Enter gross salary'}
          isOpen={open === 'income'}
          onToggle={() => toggle('income')}
          done={!!form.grossSalary}
        >
          <Text style={styles.label}>Gross salary (INR)</Text>
          <TextInput style={styles.input} value={form.grossSalary} onChangeText={v => set('grossSalary', v)}
            keyboardType="numeric" placeholder="1200000" placeholderTextColor={colors.muted} />
        </Accordion>

        <Accordion
          title="Old-regime deductions"
          subtitle="80C, 80D, NPS, HRA, home loan"
          isOpen={open === 'deductions'}
        ...
        >
          {DEDUCTIONS.map(item => (
            <View key={item.key} style={styles.inputGroup}>
              ...
              <TextInput style={styles.input} value={form[item.key]} onChangeText={v => set(item.key, v)}
        ...
        {result && (
          <>
            <View style={[styles.recCard, { borderColor: result.recommendation === 'new' ? colors.accent : colors.warning }]}>
              <Text style={styles.recLabel}>Recommendation</Text>
              <Text style={[styles.recValue, { color: result.recommendation === 'new' ? colors.accent : colors.warning }]}>
            ...
            <View style={styles.compRow}>
              <View style={[styles.compCard, result.recommendation === 'old' && styles.compCardWinner]}>
                ...
                <Text style={styles.compTaxable}>Taxable: INR {result.oldRegime.taxableIncome.toLocaleString()}</Text>
                <Text style={styles.compTax}>INR {result.oldRegime.total.toLocaleString()}</Text>
              ...
              <View style={[styles.compCard, result.recommendation === 'new' && styles.compCardWinner]}>
                ...
                <Text style={styles.compTaxable}>Taxable: INR {result.newRegime.taxableIncome.toLocaleString()}</Text>
                <Text style={styles.compTax}>INR {result.newRegime.total.toLocaleString()}</Text>
                <Text style={styles.compBreak}>
...
const styles = StyleSheet.create({
  ...
  recCard: { ...card, marginBottom: spacing.md, alignItems: 'center', paddingVertical: spacing.xl },
